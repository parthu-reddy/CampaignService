...
/*
 * Contract for AD_CAMPAIGN_DELETED event emitted by CampaignService.
 * Uses the canonical CampaignChangedEvent schema.
 */
org.springframework.cloud.contract.spec.Contract.make {
    description("Should publish AD_CAMPAIGN_DELETED to ad-events")
    label("ad_events_deleted")
    input { triggeredBy('fireAdEventDeleted()') }
    outputMessage {
        sentTo('ad-events')
        headers {
            header('eventType', 'AD_CAMPAIGN_DELETED')
