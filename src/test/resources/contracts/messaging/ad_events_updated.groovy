...
/*
 * Contract for AD_CAMPAIGN_UPDATED event emitted by CampaignService.
 * Uses the canonical CampaignChangedEvent schema.
 */
org.springframework.cloud.contract.spec.Contract.make {
    description("Should publish AD_CAMPAIGN_UPDATED to ad-events")
    label("ad_events_updated")
    input { triggeredBy('fireAdEventUpdated()') }
    outputMessage {
        sentTo('ad-events')
        headers {
            header('eventType', 'AD_CAMPAIGN_UPDATED')
