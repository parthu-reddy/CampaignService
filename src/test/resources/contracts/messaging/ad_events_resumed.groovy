...
/*
 * Contract for AD_CAMPAIGN_RESUMED event emitted by CampaignService.
 * Uses the canonical CampaignChangedEvent schema.
 */
org.springframework.cloud.contract.spec.Contract.make {
    description("Should publish AD_CAMPAIGN_RESUMED to ad-events")
    label("ad_events_resumed")
    input { triggeredBy('fireAdEventResumed()') }
    outputMessage {
        sentTo('ad-events')
        headers {
            header('eventType', 'AD_CAMPAIGN_RESUMED')
